...
#pragma region 2
		// 新しい経験を選んだ場合
		 { "もし全ての新しい経験が必ず後悔すると知っていた場合でも、君は「停滞」を\n"
			"避けるためにその選択肢を選ぶ？",
			{ {"選ぶ", 3, 480, 760},
			{"選ばない", 3, 1290, 760}}
			},
...
#pragma region 4
		// 押せるを選んだ場合
		 {"ボタンを押した結果、新しい人生を生きる君は「今の幸福な自分」を知らない\n"
			"大切な誰かに、「無かったことにした自分」の思い出を語れる？",
			{ {"語れる", 6, 450, 760},
			{"語れない", 6, 1290, 760}}
			},
		// 押せないを選んだ場合
		{"もし君の大切な誰かが、君の過去の失敗によって今も苦しみ続けていたら、\n"
		  "君はその「誰かの痛み」を見て、なお自分の幸福を優先できる？",
			{ {"できる", 6, 450, 760},
			{"できない", 6, 1290, 760} }
			},
...
#pragma region 5
		...
		#pragma region 6
		// 完璧な計画を選んだ場合
		 {"「完璧な計画」の完成を待つことで、それを実行に移すタイミングが永久に\n"
		   "失われた場合、その計画は「失敗」と見なすべき？",
			{ {"見なす",9, 450, 760},
			{"見なさない", 9, 1265, 760}}
			},
		// 即座の行動を選んだ場合
		{"即斬に行動することで成果を得られる可能性が高い一方で、修正不可能な\n"
		  "致命的なミスが、行動開始から1分以内に発生することが確実である場合、\n"
		  "それでも、あなたは「即座に行動」を優先する？",
			{ {"優先する", 9, 430, 760}, // 文章を考え直す
			{"優先しない", 9, 1265, 760} }
			},
#pragma region 7
		{"もし君が無気力で夢がない場合、時間をかけて探す？それとも、外部からの\n"
		  "強制に素直に従う？",
			{{"探す" , 10, 480, 760} ,
			{ "素直に従う" , 11, 1265, 760 }}
			},
...
#pragma region 9
		{"もし自分の命日がわかってしまい、それが避けられない運命だった場合、君は\n"
		  "その運命に抗って長生きをしようとする？それとも、おとなしく受け入れる？",
			{{"抗う" , 13, 480, 760} ,
			{ "受け入れる" , 14, 1265, 760 }}
			},
#pragma region 10
...
#pragma region 1
	...
	{{"なるほど、君は変化を恐れないチャレンジャーなんだね。",
		...
		"なら、その新しい経験が、「後悔」を伴うと確実な場合でも、君は立ち止まら\n"
		"ないのかな？"},0, 0},
		// 安心感を選んだ場合
...
#pragma region 2
	...
	{{"なるほど。\n"
		"「自由」よりも「平穏」を選んだんだね。",
		"君は全ての可能性を閉ざしても、不安のない日々こそが真の幸福だと思ってる\n"
		"んだね！"}, 2, 0},
		// 呼べないを選んだ場合
	{{"そうなんだ。\n"
...
#pragma region 3
		// 押せるを選んだ場合
		{{"なるほど。君はボタンを押すんだね。",
			"つまり、君は「現在の幸福」も「過去の失敗の代償」だと捉えてるってことな\n"
			"んだね。",
...
#pragma region 5
	...
	{{"スピードを重視する、決断力のあるタイプなんだね！",
		"でも、「修正不可能な致命的なミス」が1分以内に起こる確実な場合、それでも\n"
		"君は即座に行動を優先する？"},6, 1},
	...
	{{"君は失敗と見なさないんだね。",
		...
		// 優先するを選んだ場合
	{{"君はスピードを優先するんだね！",
		"致命的な結果が待っていても、スピードそのものに価値を見出したんだね。"}, 8, 0},
		// 優先しないを選んだ場合
	{{"なるほど。スピードが大事でも優先はしないんだね。",
		"やはり、結果の重大さを考えれば、一時停止の勇気が必要ということだね。"}, 8, 1},
#pragma region 7
		// 内部からの探求を選んだ場合
	...
	{{"決断のエネルギーを他者に委ねる、ある意味で合理的な判断だね。",
		"その強制された目標を完璧に達成し、周囲が君の幸せを保証した場合、君は\n"
		"そこに「自己の選択」を見出すだろうか？"},9, 1},
...
#pragma region 8
	...
	{{"そっか。君は有意義ではないという意見なんだね。",
		...
		// 内部からの探求を選んだ場合
	{{"なるほどね。",
		"君は与えられた道でも、努力と達成を通じて、君自身の道になったんだね。"}, 11, 0},
		// 外部からの強制に素直に従うを選んだ場合
	{{"そっか。見出さないもしくは見出せないってところかな？",
		"他者からの評価や達成度に関わらず、「自らの意志」がなければ、それは偽り\n"
		"の人生ということだね。"}, 11, 1},
#pragma region 9
		// 抗うを選んだ場合
	{{"運命に屈しない、強い意志だね。\n"
	...
	{{"受け入れることの潔さ、もしくは諦めたってことかな。",
		"なら、受け入れて残された時間を「最大限に充実させる」努力は、運命への\n"
		"「抵抗」と呼べると思う？"},12, 1},

#pragma region 10
		// 言えるを選んだ場合
	{{"なるほど。",
		"運命に抗う行為が結果的に運命を予定より早めてしまったけど、君はそれでも\n"
		"正しい行動だったという結論なんだね。"}, 13, 0},
		// 言えないを選んだ場合
	{{"言えないを選択したんだね。",
		"君は、結果的に予定より早まった以上、その行動を正しいとは言い切れないと\n"
		"感じだんだね。"
		"つまり、意図と結果を切り分けて考えた判断をしたってことか。"}, 13, 1},//文章を考え直す
		// 呼べるを選んだ場合
	{{"なるほど。君はこっちを選んだんだね。",
		"運命の日を変えられなくても、その中身を濃くすることは、受動的な死への\n"
		"抵抗だということか。"}, 14, 0},
		// 呼べないを選んだ場合
	{{"そうだよね。呼べないよね。",
		"運命の日を受け入れた以上、残りの時間の使い方を変えても、それはあくまで\n"
		"運命の範疇ということだね。"}, 14, 1},
	};
